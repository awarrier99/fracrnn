...
cfile = torch.DiskFile('cstates.csv', 'w')
hfile = torch.DiskFile('hstates.csv', 'w')
...
while true do
	if i == opt.length + 1 then break end
    ...
    let = ivocab[prev_char[1]]
    if let == '\n' then
        let = '\\'
    end
    cfile:writeString(let)
    hfile:writeString(let)
    for i=1,state_size do table.insert(current_state, lst[i])
        lstitbl = lst[i]:totable()
        for j=1,#lstitbl do
            for k=1,#lstitbl[j] do
                if (i % 2) == 1 then cfile:writeString(string.format(',%+f', lstitbl[j][k])) end
                if (i % 2) == 0 then hfile:writeString(string.format(',%+f', lstitbl[j][k])) end
            end
        end
    end
    cfile:writeString('\n')
    ...
    io.write(ivocab[prev_char[1]])
	if tonumber(ivocab[prev_char[1]]) then 
		if not tonumber(prev_ch) then
			i = i + 1
		end
	end
	prev_ch = ivocab[prev_char[1]]
end
cfile:close()
hfile:close()
io.write('\n') io.flush()
